...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    MENU_LABEL = _("&Leopard speech engine...")
    MENU_HELP = _("Check whether leopardspeech can find its engine, and open the "
                  "folder it goes in.")
    ...
    def _onMenu(self, evt):
        ...
        log.info("Leopard-speech: engine %s (from the Tools menu)\n  %s"
        ...
        if ok:
            gui.messageBox(
                "leopardspeech has its engine.\n\n%s" % "\n".join(lines),
                "Leopard-speech", wx.OK | wx.ICON_INFORMATION)
            return
        self._ask(folder)
        ...
        try:
            ...
            if not os.path.exists(readme):
                with open(readme, "w", encoding="utf-8") as f:
                    f.write(_README)
            if not _claim_the_startup_dialog("leopardspeech"):
                log.info("Leopard-speech: another Macintosh speech add-on has already\n"
