...
and every rule at the top of `leopardspeech.py` was paid for there.  The fakes
...
def _stage_tree(cfg_dir):
    ...
    for c in ([env] if env else []):
        if c and os.path.isdir(os.path.join(c, "Speech", "Voices")):
            ...
            with open(os.path.join(cfg_dir, "leopardspeech-data.txt"), "w",
                      encoding="utf-8") as f:
                f.write(c)
            return c
    return None
...
def tiger_tree():
    ...
    if not os.path.isfile(leopardspeech.HOST_EXE):
        pytest.skip("leopard_host.exe not built; run sh build.sh")
    return tree
