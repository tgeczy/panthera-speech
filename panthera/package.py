...
Leopard, and each engine is read from wherever they extracted it and never
ships here.

The two lists below were maintained separately by the Tiger and Leopard
package scripts and had stayed identical, which is luck rather than a
guarantee -- both installs hold the same file names because it is the same
software four years apart.
"""
import os
import sys
import zipfile
...
#: Every name here is a real file in a Tiger or Leopard install, added as the
#: extraction turned each one up. `.i386`/`.ppc` are the slices a fat Mach-O is split into
...
def manifest(key):
    """A value out of the manifest. -> str

    The add-on name comes from there rather than a literal here, because
    the two have to agree: the plugin offers to remove add-ons whose names
    are not this one, and a package built under an old name would be a
    build that offers to remove itself.
    """
    for line in open(os.path.join(ADDON, "manifest.ini"), encoding="utf-8"):
        if line.strip().startswith(key):
            return line.split("=", 1)[1].strip()
    raise SystemExit("no %s in manifest.ini" % key)
...
def main():
    out = os.path.join(ROOT, "%s-%s.nvda-addon"
