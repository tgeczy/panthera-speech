...
Not a bridge and not an emulator.  `leopard_host.exe` is a 32-bit process that
...
Nothing of Apple's ships here.  The user supplies their own Leopard install; the
...
codecs.register_error("leopardspeech_fold", _unmappable)
...
def _encode(text):
    ...
    **The engine's text is a single-byte Mac encoding, not UTF-8.**  Sent as
    ...
    """
    return text.translate(_FOLD).encode("mac_roman", "leopardspeech_fold")
...
import pantheraleopard as tree                                # noqa: E402
...
import pantheranumbers as leopardnumbers                      # noqa: E402
...
_MISSING = (
    "Leopard speech cannot start, because the engine is not there yet.\n\n"
    "This add-on ships no part of Apple's software. You supply it from your "
    "own Mac OS X 10.5 install disc, and put the extracted Speech folder and "
...
def _explainLater(folder):
    ...
    It lands on top of NVDA's own "Could not load the leopardspeech synthesizer"
    ...
    def show():
        try:
            answer = gui.messageBox(_MISSING % folder, "Leopard speech",
...
class SynthDriver(SynthDriver):
    ...
    def __init__(self):
        ...
        if not ok:
            log.warning("leopard-speech cannot start:\n  %s" % "\n  ".join(lines))
            _explainLater(tree.config_dir())
            raise RuntimeError("leopard-speech has no engine to run")
        self._tree = find_tree()
        if not self._tree:
            raise RuntimeError("no Leopard speech tree found")
        ...
        self._feeder = threading.Thread(target=self._feed,
                                        name="leopardspeech-feed", daemon=True)
        self._feeder.start()
        self._worker = threading.Thread(target=self._run, name="leopardspeech",
                                        daemon=True)
        self._worker.start()
    ...
    def _makeCancelEvent(self):
        ...
        try:
            ...
            name = "Local\\leopardspeech-cancel-%d-%d" % (os.getpid(), id(self))
            ...
            log.debugWarning("leopardspeech: no cancel event (%s)" % e)
        ...
        def pump():
            try:
                ...
                try:
                    ...
                    pass
        t = threading.Thread(target=pump, name="leopardspeech-host-log")
        t.daemon = True
        t.start()
    ...
    def _feed(self):
        ...
        while not self._stopped:
            ...
            try:
                ...
                log.debugWarning("leopardspeech: feeding audio: %s" % e)
