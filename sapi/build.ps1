...
# The settings reader ships in the DLL too.  settings_test.cpp links the same
# module and proves the file format, the order of the sources, the typed
# fall-through and that a rewrite reaches the next lookup, with both files
# pointed at scratch paths and the registry redirected, so nobody's settings
# are read or written.
$settingsDir = Join-Path $env:TEMP "panthera-settings-test"
