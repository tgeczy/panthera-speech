...
def test_streamed_audio_is_identical_to_blocking(tiger_tree, voice, wpm):
    """The whole point: a different transport, the same samples."""
    proc = _host(tiger_tree)
    try:
...
def test_a_stale_host_refuses_a_streaming_request(tiger_tree):
...
def test_streaming_still_ends_cleanly_on_an_empty_utterance(tiger_tree):
