...
| [**snowleopardspeech**](snowleopard/README.md) | Mac OS X 10.6 Snow Leopard | twenty-four, including the rebuilt **Alex** |
...
**Snow Leopard is what that was for.** 10.6 is a hybrid — Lion's binding and
threading, Leopard's speech API and maths — so every piece of it already
existed, and the engine mapped, bound 1,170 symbols with none unresolved and
opened a speech channel *with no change to the host at all*. Making it speak
took two fixes, and both were in code written for one generation that had
never met the other: a `dispatch_walltime` deadline read against a monotonic
clock, and a source table sized for an engine that keeps its sources rather
than one that makes ninety-five an utterance. That is the shape the interface
was supposed to produce, and it is the argument for not branching.
...
snowleopard/  Snow Leopard's extractor and engine notes
...
`tests/tiger/`, `tests/leopard/`, `tests/snowleopard/` and `tests/lion/`.

The extractors stay one per generation, under `tiger/`, `leopard/`,
`snowleopard/` and `lion/`, because they read different install images by
different means — and because the dialogs that name their URLs have already
shipped. Snow Leopard's is the short one: since the add-on grew a disc reader
of its own for the Tools menu, there is no longer any reason for a fourth copy
of one, so it asks `pantheradiscs` and is a hundred lines rather than eight.
...
| [`macintalk-3.10.md`](docs/macintalk-3.10.md) | Snow Leopard. The hybrid: what it takes from each neighbour, the two clocks, the dispatch source per unit of work, and the libstdc++ version number two releases share and do not agree on. |
