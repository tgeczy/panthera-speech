...
class EngineSmokeTest : Instrumentation() {
    ...
    override fun onStart() {
        ...
        if (latency == "completion") { PantheraWorkerCheck.runCompletionProbe(this); return }
        ...
        val generation = testGeneration ?: PantheraEngine.activeGen(targetContext)
        val volumeKey = PantheraEngine.settingKey("volume", generation)
        val rateKey = PantheraEngine.settingKey("rate_wpm", generation)
        val baselineVolume = if (generation == "tiger") 100 else 50
        prefs.edit().putString(PantheraEngine.PREF_GEN, generation)
            .putBoolean("override_voice", false).putInt(volumeKey, baselineVolume)
