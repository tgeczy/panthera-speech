...
class SettingsActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ... --es import /path/to/lion.zip` (or a content: URI) imports
    ...
    private fun importZip(source: ZipImport.Source, confirm: Boolean) {
        ...
        Thread {
            ...
            runOnUiThread {
                ...
                val root = PantheraEngine.dataRoot(this)
                ...
                val lines = plan.found.map { f ->
                    val label = PantheraEngine.genLabel(f.gen!!)
                    "Will import into $label: MacinTalk ${f.version}, ${f.voices} voices, " +
    ...
    private fun attachImport(job: ImportJob) {
        ...
        val names = job.plan.gens.joinToString(" and ") { PantheraEngine.genLabel(it) }
        val box = column()
    ...
    private fun finishImport(job: ImportJob) {
        ...
        result.onSuccess { gens ->
            PantheraEngine.imported(this, gens)
            refresh(); rebuildVoices(); refreshVoiceButton(); rebuildEngineSwitches()
            val said = gens.joinToString(" and ") { g ->
                "${PantheraEngine.genLabel(g)} (${PantheraEngine.scanVoices(this, g).size} voices)"
            }
            importStatus.text = "Imported $said. " +
                if (PantheraEngine.verified(this)) "The engine is verified and ready."
            ...
            if (e is ZipImport.Cancelled) {
                ...
                Log.e("Panthera", "import failed", e)
    ...
    private fun buildSetup(root: LinearLayout) {
        ...
        // drive, and the app reads which engine is inside rather than
        // trusting the zip's name.
        root.addView(body(
    ...
    /** One switch per generation this app knows; rebuilt after an import,
